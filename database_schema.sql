...
CREATE TABLE epg_virtual (
    ...
    -- Garante que não haja sobreposições de programação no mesmo canal
    CONSTRAINT no_overlapping_programs EXCLUDE (
        channel_id WITH =,
        TSTZRANGE(start_time_virtual, end_time_virtual) WITH &&
    )
);
...
CREATE TRIGGER set_timestamp
BEFORE UPDATE ON channel_master_grid
FOR EACH ROW
EXECUTE PROCEDURE trigger_set_timestamp();
