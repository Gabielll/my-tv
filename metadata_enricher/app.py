#!/usr/bin/env python3
"""
Metadata Enricher Web Service

Converts the metadata_enricher worker into a web service for Render deployment.
...
def process_enrichment_job(message_body: str) -> bool:
    ...
    global last_job_processed, total_jobs_processed
...
# --- Worker Function ---
def metadata_enricher_worker():
    """
    Main worker function that runs the metadata enricher logic.
    This function runs in a background thread within the web service.
    """
    global logger
    
    # Get logger instance
    logger = get_logger('metadata-enricher')
...
def custom_health_check():
    """Custom health check for metadata enricher service."""
    health_info = {
        'apis': api_status,
        'processing': {
            'total_jobs_processed': total_jobs_processed,
            'last_job_processed': last_job_processed,
            'last_job_ago_seconds': time.time() - last_job_processed if last_job_processed else None
        }
    }
    ...
    if TMDB_API_KEY:
        try:
            response = requests.get(f"{TMDB_BASE_URL}/configuration", params={
                'api_key': TMDB_API_KEY
            }, timeout=5)
            health_info['apis']['tmdb_status'] = 'ok' if response.status_code == 200 else 'error'
        except Exception as e:
            health_info['apis']['tmdb_status'] = f'error: {str(e)}'
    ...
    if GEMINI_API_KEY:
        try:
            ...
            health_info['apis']['gemini_status'] = 'ok' if response.status_code == 200 else 'error'
        except Exception as e:
            health_info['apis']['gemini_status'] = f'error: {str(e)}'
...
if __name__ == '__main__':
    ...
    service = create_worker_web_service(
        service_name='metadata-enricher',
        worker_function=metadata_enricher_worker,
